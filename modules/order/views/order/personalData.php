            ...
            if (saveJPGButton) {
                saveJPGButton.addEventListener('click', () => {
                    if (signaturePad.isEmpty()) {
                        showMessage('Пожалуйста, сначала нарисуйте подпись.', 'error');
                    ...
                    const fullName = '<?php echo htmlspecialchars($full_name); ?>';
                    const idPep = '<?php echo htmlspecialchars($id_pep); ?>';
                    const headerText = '<?php echo addslashes(str_replace('{full_name}', $full_name, $consent_text)); ?>';
                    ...
                    console.log('JPG сгенерирован, отправляется на сервер...');

                    const saveUrl = '<?php echo URL::site("order/save_signature/".$id_pep); ?>';
                    fetch(saveUrl, {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            showMessage('Подпись успешно сохранена на сервере! ' + data.file, 'success');
                            console.log('Подпись сохранена на сервере:', data.file);
                        } else {
                    ...
                    .catch(error => {
                        showMessage('Ошибка: Сохранение на сервер недоступно в оффлайн-режиме.', 'error');
